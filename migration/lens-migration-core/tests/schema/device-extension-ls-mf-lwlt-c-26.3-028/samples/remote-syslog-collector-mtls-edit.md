This macro configures a remote syslog collector over TLS with mutual authentication. The end entity certificate that is going to be used during the handshake phase is configured with this macro as part of local-definition container. As next step the private key must be installed via the relevant action (refer to macro system-logging-remote-syslog-collector-tls-install-private-key). In case of dynamic certificate enrollment via an external PKI server using EST as protocol, a reference is used instead, both for end entity certificate and private key.
...
* certificate: The certificate to be used for syslog
* cipher-suite: YANG identifier of a TLS cipher to add
* client-auth-est-profile-name: The PKI EST profile name to be used for client authentication, if parameter 'certificate' is empty.
