This macro configures a remote syslog collector over TLS with oneway authentication. This means
that LightSpan node can verify syslog server but the server cannot verify the access node (no key pair configured on LightSpan node).
