...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:bbf-qos-filt="urn:bbf:yang:bbf-qos-filters" xmlns:bbf-qos-enhfilt="urn:bbf:yang:bbf-qos-enhanced-filters" xmlns:bbf-qos-pol="urn:bbf:yang:bbf-qos-policies" xmlns:bbf-qos-cls="urn:bbf:yang:bbf-qos-classifiers" xmlns:bbf-qos-plc="urn:bbf:yang:bbf-qos-policing" xmlns:nokia-qos-filt="http://www.nokia.com/Fixed-Networks/BBA/yang/nokia-qos-filters-ext" xmlns:nokia-sdan-qos-policing-extension="http://www.nokia.com/Fixed-Networks/BBA/yang/nokia-sdan-qos-policing-extension" xmlns:nokia-qos-cls-ext="http://www.nokia.com/Fixed-Networks/BBA/yang/nokia-sdan-qos-classifier-extension" version="1.0">
  ...
  <xsl:template match="*[     local-name() = 'policy-profile'     and parent::*[local-name() = 'qos-policy-profiles' and namespace-uri() = 'urn:bbf:yang:bbf-qos-policies']   ]">
    <xsl:copy>
      <xsl:copy-of select="@*"/>
          ...
          <xsl:element name="policies">
            <xsl:for-each select="child::*[local-name() = 'policy-list']">
              <xsl:element name="policy">
                ...
                <xsl:element name="enhanced-filters"/>
                <xsl:variable name="curPolicy" select="//*[                   local-name() = 'policy'                   and parent::*[local-name() = 'policies' and namespace-uri() = 'urn:bbf:yang:bbf-qos-policies']                   and child::*[local-name() = 'name'] = $curPolicyName                 ]"/>
                      ...
                      <xsl:element name="filters">
                        <xsl:if test="$classifier/child::*[local-name() = 'enhanced-filter-name' and namespace-uri() = 'urn:bbf:yang:bbf-qos-enhanced-filters']">
                          <xsl:element name="ref-filter">
                            ...
                            <xsl:variable name="enhancedFilter" select="//*[                               local-name() = 'enhanced-filter' and namespace-uri() = 'urn:bbf:yang:bbf-qos-enhanced-filters'                               and parent::*[local-name() = 'filters' and namespace-uri() = 'urn:bbf:yang:bbf-qos-filters']                               and child::*[local-name() = 'name'] = $refFilterName                             ]"/>
