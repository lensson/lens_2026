...
<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="kex-valid-values_" select="'diffie-hellman-group14-sha256,diffie-hellman-group-exchange-sha256'"/>
  ...
  <xsl:template match="dyncfg-ns:confdConfig/dyncfg-ns:ssh/dyncfg-ns:algorithms">
    <xsl:copy>
      ...
      <xsl:if test="not(dyncfg-ns:kex)">
        ...
        <xsl:element name ="kex" namespace="{namespace-uri()}">
      ...
      <xsl:apply-templates select="*[not(self::dyncfg-ns:kex)]"/>
    </xsl:copy>
  </xsl:template>
