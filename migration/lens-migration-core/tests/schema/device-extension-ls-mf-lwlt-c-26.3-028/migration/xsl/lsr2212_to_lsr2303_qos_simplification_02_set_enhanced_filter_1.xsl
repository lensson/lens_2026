...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:bbf-qos-filt="urn:bbf:yang:bbf-qos-filters" xmlns:bbf-qos-enhfilt="urn:bbf:yang:bbf-qos-enhanced-filters" xmlns:bbf-qos-pol="urn:bbf:yang:bbf-qos-policies" xmlns:bbf-qos-cls="urn:bbf:yang:bbf-qos-classifiers" xmlns:bbf-qos-plc="urn:bbf:yang:bbf-qos-policing" xmlns:nokia-qos-filt="http://www.nokia.com/Fixed-Networks/BBA/yang/nokia-qos-filters-ext" xmlns:nokia-sdan-qos-policing-extension="http://www.nokia.com/Fixed-Networks/BBA/yang/nokia-sdan-qos-policing-extension" xmlns:nokia-qos-cls-ext="http://www.nokia.com/Fixed-Networks/BBA/yang/nokia-sdan-qos-classifier-extension" xmlns="" version="1.0">
  ...
  <xsl:template match="*[     local-name() = 'enhanced-filters'     and parent::*[local-name() = 'current']     and ancestor::*[local-name() = 'migration-cache']     and ancestor::*[local-name() = 'policy-profile']     and ancestor::*[local-name() = 'qos-policy-profiles' and namespace-uri() = 'urn:bbf:yang:bbf-qos-policies']   ]">
    <xsl:variable name="cur" select=".."/>
    <xsl:copy>
      <xsl:copy-of select="@*"/>
<xsl:for-each select="//*[       local-name() = 'enhanced-filter' and namespace-uri() ='urn:bbf:yang:bbf-qos-enhanced-filters'       and parent::*[local-name() = 'filters' and namespace-uri() = 'urn:bbf:yang:bbf-qos-filters']       ]">
  ...
  <xsl:template name="getFilterOperation">
    ...
    <xsl:if test="$filterSec/child::*[local-name() = 'filter-operation']">
      <xsl:variable name="operationVar">
        <xsl:value-of select="normalize-space($filterSec/child::*[local-name() = 'filter-operation'])"/>
      </xsl:variable>
      <xsl:choose>
  ...
  <xsl:template name="getEnhancedFilterType">
    ...
    <xsl:variable name="flowColorSec" select="$enhancedFilter/child::*[local-name() = 'filter']/child::*[local-name() = 'flow-color']"/>
    <xsl:choose>
  ...
  <xsl:template name="getEnhancedFilterRefFilter">
    <xsl:param name="enhancedFilterName"/>
    <xsl:variable name="enhancedFilter" select="//*[       local-name() = 'enhanced-filter' and namespace-uri() = 'urn:bbf:yang:bbf-qos-enhanced-filters'       and parent::*[local-name() = 'filters' and namespace-uri() = 'urn:bbf:yang:bbf-qos-filters']       and child::*[local-name() = 'name' and text() = $enhancedFilterName]     ]"/>
    <xsl:variable name="refFilterSec" select="$enhancedFilter/child::*[local-name() = 'filter']/child::*[local-name() = 'enhanced-filter-name']"/>
