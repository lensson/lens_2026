...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <!--xsl:template match="/cfgNs:config/if:interfaces/if:interface/portRef:port-layer-if[../if:type='bbf-xponift:channel-pair' or
                                             ...
                                             ../if:type='bbf-xponift:v-ani']"/ to be added -->
  ...
  <!-- Rule 4： Not support xpongemtcont:tcont-ref -->
  <!--delete not support /bbf-xpongemtcont:xpongemtcont/bbf-xpongemtcont:gemports/bbf-xpongemtcont:gemport/bbf-xpongemtcont:tcont-ref. -->
  <xsl:template match="/cfgNs:config/tcont:xpongemtcont/tcont:gemports/tcont:gemport/tcont:tcont-ref"/>


  <!--Rule 8: Must always be true for olt-v-enet. -->
  <xsl:template match="/cfgNs:config/if:interfaces/if:interface/if:enabled[.='false' and ../if:type='bbf-xponift:olt-v-enet']">
      <xsl:element name="enabled" namespace="{namespace-uri()}">true</xsl:element>
  </xsl:template>
