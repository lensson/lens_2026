...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:bbf-qos-filt="urn:bbf:yang:bbf-qos-filters" xmlns:bbf-qos-enhfilt="urn:bbf:yang:bbf-qos-enhanced-filters" xmlns:bbf-qos-pol="urn:bbf:yang:bbf-qos-policies" xmlns:bbf-qos-cls="urn:bbf:yang:bbf-qos-classifiers" xmlns:bbf-qos-plc="urn:bbf:yang:bbf-qos-policing" xmlns:nokia-qos-filt="http://www.nokia.com/Fixed-Networks/BBA/yang/nokia-qos-filters-ext" xmlns:nokia-sdan-qos-policing-extension="http://www.nokia.com/Fixed-Networks/BBA/yang/nokia-sdan-qos-policing-extension" xmlns:nokia-qos-cls-ext="http://www.nokia.com/Fixed-Networks/BBA/yang/nokia-sdan-qos-classifier-extension" xmlns="" version="1.0">
  ...
  <xsl:template match="*[     local-name() = 'ref-by'     and parent::*[local-name() = 'enhanced-filter']     and ancestor::*[local-name() = 'enhanced-filters']     and ancestor::*[local-name() = 'current']     and ancestor::*[local-name() = 'migration-cache']     and ancestor::*[local-name() = 'policy-profile']     and ancestor::*[local-name() = 'qos-policy-profiles' and namespace-uri() = 'urn:bbf:yang:bbf-qos-policies']   ]">
    <xsl:variable name="cur" select="../../.."/>
    <xsl:variable name="curEnhancedFilterSec" select=".."/>
    <xsl:variable name="curEnhancedFilterName" select="../name"/>
    <xsl:copy>
      <xsl:copy-of select="@*"/>
<xsl:for-each select="$cur/policies/policy/classifiers/classifier/filters/ref-filter[ref = $curEnhancedFilterName and type = $F_EN_FILTER_FLOW_COLOR]">
        <xsl:variable name="usedInClassifierName">
        ...
        <xsl:element name="filter">
          ...
          <xsl:variable name="childFilter" select="//*[             local-name() = 'enhanced-filter' and namespace-uri() ='urn:bbf:yang:bbf-qos-enhanced-filters'             and parent::*[local-name() = 'filters' and namespace-uri() = 'urn:bbf:yang:bbf-qos-filters']             and child::*[local-name() = 'name'] = $curChildFilterName           ]"/>
          ...
          <xsl:element name="use-in-classifier">
            <xsl:value-of select="$usedInClassifierName"/>
          </xsl:element>
        </xsl:element>
      </xsl:for-each>
    </xsl:copy>
  ...
  <xsl:template match="*[     local-name() = 'classifier'     and parent::*[local-name() = 'classifiers']     and ancestor::*[local-name() = 'classifiers']     and ancestor::*[local-name() = 'policy']     and ancestor::*[local-name() = 'current']     and ancestor::*[local-name() = 'policy-profile']     and ancestor::*[local-name() = 'qos-policy-profiles' and namespace-uri() = 'urn:bbf:yang:bbf-qos-policies']   ]">
    ...
    <xsl:variable name="classifier" select="//*[     local-name() = 'classifier-entry'     and parent::*[local-name() = 'classifiers' and namespace-uri() = 'urn:bbf:yang:bbf-qos-classifiers']     and child::*[local-name() = 'name'] = $curClsName     ]"/>
...
<xsl:if test="$filterOperationVar and string-length($filterOperationVar) &gt; 0">
  ...
  <xsl:template match="       *[       local-name() = 'actions'       and parent::*[local-name() = 'classifier']       and ancestor::*[local-name() = 'classifiers']       and ancestor::*[local-name() = 'policies']       and ancestor::*[local-name() = 'current']       and ancestor::*[local-name() = 'migration-cache']       and ancestor::*[local-name() = 'policy-profile']       and ancestor::*[local-name() = 'qos-policy-profiles' and namespace-uri() = 'urn:bbf:yang:bbf-qos-policies']       ]">
...
<xsl:for-each select="$classifier/child::*[local-name() = 'classifier-action-entry-cfg']">
  ...
  <xsl:template match="       *[       local-name() = 'filters'       and parent::*[local-name() = 'classifier']       and ancestor::*[local-name() = 'classifiers']       and ancestor::*[local-name() = 'policies']       and ancestor::*[local-name() = 'current']       and ancestor::*[local-name() = 'migration-cache']       and ancestor::*[local-name() = 'policy-profile']       and ancestor::*[local-name() = 'qos-policy-profiles' and namespace-uri() = 'urn:bbf:yang:bbf-qos-policies']       ]">
    <xsl:variable name="curClsName">
      <xsl:value-of select="../child::*[local-name() = 'name']"/>
    </xsl:variable>
    <xsl:variable name="classifier" select="//*[     local-name() = 'classifier-entry'     and parent::*[local-name() = 'classifiers' and namespace-uri() = 'urn:bbf:yang:bbf-qos-classifiers']     and child::*[local-name() = 'name'] = $curClsName     ]"/>
    <xsl:variable name="inlineClassifierTypeVar">
      <xsl:if test="$classifier/child::*[local-name() = 'match-criteria']">
        <xsl:call-template name="getInlineFilterType">
          <xsl:with-param name="matchCriteriaSec" select="$classifier/child::*[local-name() = 'match-criteria']"/>
        </xsl:call-template>
      </xsl:if>
    </xsl:variable>
...
<xsl:if test="$inlineClassifierTypeVar and string-length($inlineClassifierTypeVar) &gt; 0">
      ...
      <xsl:if test="         (contains($inlineClassifierTypeVar,$F_FLOW_COLOR) and (not($enhClassifierTypeVar) or $enhClassifierTypeVar = ''))         or (contains($enhClassifierTypeVar,$F_FLOW_COLOR) and (not($inlineClassifierTypeVar) or $inlineClassifierTypeVar = ''))       ">
        <xsl:element name="self-filter-flow-color">
          <xsl:if test="contains($inlineClassifierTypeVar, $F_FLOW_COLOR)">
            <xsl:variable name="flowColorVar">
              <xsl:value-of select="$classifier/child::*[local-name() = 'match-criteria']/child::*[local-name() = 'flow-color']"/>
            </xsl:variable>
            <xsl:choose>
...
<xsl:if test="$filterOperationVar and string-length($filterOperationVar) &gt; 0">
  ...
  <!-- Insert info on classifier filters -->
  <xsl:template match="       *[       local-name() = 'filters'       and parent::*[local-name() = 'classifiers']       and ancestor::*[local-name() = 'policy-migration-cache']       and ancestor::*[local-name() = 'policy']       and ancestor::*[local-name() = 'policies' and namespace-uri() = 'urn:bbf:yang:bbf-qos-policies']       ]">
    <xsl:variable name="curClsName">
      <xsl:value-of select="../child::*[local-name() = 'name']"/>
    </xsl:variable>
...
<xsl:if test="$inlineClassifierTypeVar and string-length($inlineClassifierTypeVar) &gt; 0">
  ...
  <!-- Insert info on classifier actions -->
  <xsl:template match="       *[       local-name() = 'actions'       and parent::*[local-name() = 'classifiers']       and ancestor::*[local-name() = 'policy-migration-cache']       and ancestor::*[local-name() = 'policy']       and ancestor::*[local-name() = 'policies' and namespace-uri() = 'urn:bbf:yang:bbf-qos-policies']       ]">
    <xsl:variable name="curClsName">
      <xsl:value-of select="../child::*[local-name() = 'name']"/>
    </xsl:variable>
...
<xsl:for-each select="$classifier/child::*[local-name() = 'classifier-action-entry-cfg']">
  ...
  <xsl:template name="getFilterOperation">
    ...
    <xsl:if test="$classifierSec/child::*[local-name() = 'filter-operation']">
      <xsl:variable name="operationVar">
        <xsl:value-of select="normalize-space($classifierSec/child::*[local-name() = 'filter-operation'])"/>
      </xsl:variable>
      <xsl:choose>
  ...
  <xsl:template name="getActionType">
    ...
    <xsl:choose>
      <xsl:when test="$localActionTypeVar = 'pbit-marking' or $localActionTypeVar = 'bbf-qos-cls:pbit-marking'">
        <xsl:value-of select="$A_PBIT_MARKING"/>
      </xsl:when>
      <xsl:when test="$localActionTypeVar = 'dei-marking' or $localActionTypeVar = 'bbf-qos-cls:dei-marking'">
        <xsl:value-of select="$A_DEI_MARKING"/>
      </xsl:when>
      <xsl:when test="$localActionTypeVar = 'dscp-marking' or $localActionTypeVar = 'bbf-qos-cls:dscp-marking'">
        <xsl:value-of select="$A_DSCP_MARKING"/>
      </xsl:when>
      <xsl:when test="$localActionTypeVar = 'scheduling-traffic-class' or $localActionTypeVar = 'bbf-qos-cls:scheduling-traffic-class'">
        <xsl:value-of select="$A_SCHEDULING_TRAFFIC"/>
      </xsl:when>
      <xsl:when test="$localActionTypeVar = 'flow-color' or $localActionTypeVar = 'bbf-qos-plc:flow-color'">
        <xsl:value-of select="$A_FLOW_COLOR"/>
      </xsl:when>
      <xsl:when test="$localActionTypeVar = 'bac-color' or $localActionTypeVar = 'bbf-qos-plc:bac-color'">
        <xsl:value-of select="$A_BAC_COLOR"/>
      </xsl:when>
      <xsl:when test="$localActionTypeVar = 'discard' or $localActionTypeVar = 'bbf-qos-plc:discard'">
        <xsl:value-of select="$A_DISCARD"/>
      </xsl:when>
      <xsl:when test="$localActionTypeVar = 'policing' or $localActionTypeVar = 'bbf-qos-plc:policing'">
      ...
      <xsl:when test="$localActionTypeVar = 'rate-limit-frames' or $localActionTypeVar = 'bbf-qos-rc:rate-limit-frames'">
        <xsl:value-of select="$A_RATE_LIMIT"/>
      </xsl:when>
      <xsl:when test="$localActionTypeVar = 'policing-traffic-class' or $localActionTypeVar = 'nokia-qos-cls-ext:policing-traffic-class'">
        <xsl:value-of select="$A_PBIT_POLICING_TC"/>
      </xsl:when>
      <xsl:when test="$localActionTypeVar = 'count' or $localActionTypeVar = 'nokia-qos-cls-ext:count'">
  ...
  <xsl:template name="getInlineFilterType">
    ...
    <xsl:variable name="inlineFilterVar">
      ...
      <xsl:if test="$matchCriteriaSec/child::*[local-name() = 'protocol']">
        <xsl:choose>
          <xsl:when test="$matchCriteriaSec/child::*[local-name() = 'protocol' and text() = 'igmp']">
      ...
      <xsl:if test="$classifierSec/child::*[local-name() = 'ip-common']">
        ...
        <xsl:if test="$classifierSec/child::*[local-name() = 'ip-common']/child::*[local-name() = 'protocol']">
          <xsl:variable name="ipcommon_protocol">
            <xsl:value-of select="normalize-space($classifierSec/child::*[local-name() = 'ip-common']/child::*[local-name() = 'protocol'])"/>
          </xsl:variable>
          <xsl:choose>
  ...
  <xsl:template name="getPolicingProfileType">
    ...
    <xsl:variable name="policingProfile" select="//*[       local-name() = 'policing-profile'       and parent::*[local-name() = 'policing-profiles' and namespace-uri() = 'urn:bbf:yang:bbf-qos-policing']       and child::*[local-name() = 'name'] = $policingProfileName     ]"/>
    <xsl:choose>
      <xsl:when test="$policingProfile/child::*[local-name() ='two-rate-three-color-marker-mef']">
        <xsl:choose>
          <xsl:when test="$policingProfile/child::*[local-name() ='two-rate-three-color-marker-mef']/child::*[local-name() ='color-mode'] = $color-aware">
  ...
  <xsl:template name="getEnhancedFilterColor">
    ...
    <xsl:variable name="enhancedFilter" select="//*[       local-name() = 'enhanced-filter' and namespace-uri() = 'urn:bbf:yang:bbf-qos-enhanced-filters'       and parent::*[local-name() = 'filters' and namespace-uri() = 'urn:bbf:yang:bbf-qos-filters']       and child::*[local-name() = 'name' and text() = $enhancedFilterNameVar]     ]"/>
